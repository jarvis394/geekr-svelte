/**
 * Adapted from https://github.com/paoloricciuti/sveltekit-view-transition
 * MIT License
 * Copyright (c) 2023 Paolo Ricciuti
 *
 * - Fixed BF navigation in classes() function by using 'transition-ready' event
 * - Added shouldStartViewTransition option to setupViewTransition() function
 */
...
export type SetupViewTransitionOptions = {
	shouldStartViewTransition?: (navigation: OnNavigate) => boolean
}
...
export function setupViewTransition({
	...
	if (on_navigate_registered === 0 && browser && document.startViewTransition) {
		...
		onNavigate((navigation) => {
			...
			if (shouldStartViewTransition?.(navigation) === false) return
